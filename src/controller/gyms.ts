...
const createGym = (req: Request, res: Response, next: NextFunction) => {
  ...
  let query = 'INSERT INTO gyms (city, name) ';
  query += `VALUES ("${city}", "${name}")`;
...
const getTrainersByGym = (req: Request, res: Response, next: NextFunction) => {
  console.log("Getting trainers By Gym");

  let { id } = req.params;
  let query = `SELECT * FROM trainers where gym_id=${id}`;
